...
public class PlayerActivator extends GameObject {
    
    private Timer DespawnTimer;
    private boolean GoInactiveNextFrame = false;
    
    public PlayerActivator(int aitype, Global.Directions horzfacing, Global.Directions vertfacing, float locationx, float locationy, boolean respawnable, boolean respawned, int objectarrayid, float inertiax, float inertiay, String text)
    {
...
// </editor-fold>
        ...
        DespawnTimer = new Timer("Goes away next frame", 10, true, true);
    }
...
// </editor-fold>
    ...
    protected void ComputeStatus(int delta, int[][][] TileData, int[][] RoomData, PlayerCharacter Player) {
        ...
        if (DespawnTimer.Update(delta)) {
            IsAlive = false;
        }
        if (GoInactiveNextFrame) {
            IsAlive = false;
        }
    }
    ...
    {
        if (Object.IsPlayerActivatorEnabled())
        {
            //This triggers a cutscene from the thing that it hit.
            System.out.println("Player Activator hit something it cares about");
            GoInactiveNextFrame = true;
        }
