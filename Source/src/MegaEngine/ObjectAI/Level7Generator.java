 ...
 * @author McRib
 */
public class Level7Generator extends GameObject {
            
    public Level7Generator(int aitype, Global.Directions horzfacing, Global.Directions vertfacing, float locationx, float locationy, boolean solidwall, boolean deleteonactivate, int objectarrayid, float spriterow, float spritecolumn, String cutscenefilename)
    {
...
// <editor-fold defaultstate="collapsed" desc="Animation0 Default">
        ...
        EntityAnimation[0].AddFrame(0, 11, 5000, 0, 0);
...
// </editor-fold>
    ...
    protected void ComputeStatus(int delta, int[][][] TileData, int[][] RoomData, PlayerCharacter Player) {
        ...
        if (main.Global.SwitchVariables.GetVariable("SpareBattery")) {
            IsAlive = false;
        }
    }
    ...
    public void ProcessCollision(GameObject Object)
    {
        if (Object.GetIsAlive() && Objects.equals(Object.GetName(), "Player Activator"))
        {
            if (main.Global.SwitchVariables.GetVariable("ToolKit")) {
                //We got the thing!
                main.Global.SwitchVariables.SetVariable("SpareBattery", true);
                LevelState.OpenChatWindow("Level7-OpenGenerator");
                IsAlive = false;
            } else {
                LevelState.OpenChatWindow("Level7-LockedGenerator");
            }
        }
    }
}